...
func (w *watchCache) processEvent(event watch.Event, resourceVersion uint64, updateFunc func(*storeElement) error) error {
	...
	if err := func() error {
		// TODO: We should consider moving this lock below after the wcEvent
...
func (w *watchCache) WaitUntilFreshAndIndex(resourceVersion uint64, indexName string, val string, trace *utiltrace.Trace) ([]interface{}, uint64, error) {
	err := w.waitUntilFreshAndBlock(resourceVersion, trace)
	defer w.RUnlock()
	if err != nil {
		return nil, 0, err
	}
	result, err := w.store.ByIndex(indexName, val)
	if err != nil {
		return nil, 0, err
	}
	klog.Infof("suntianyuan-cache-test: index result len is %v, rv is %v", len(result), w.resourceVersion)
	return result, w.resourceVersion, nil
}
